...
export class SupplierFormComponent implements OnInit,OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if(this.id)
    {
      ...
      this.cancelShit(true);
      this.userService.getSupplierById(this.id).subscribe(response=>{
        this.supplierRequest=response;
        this.initForme1();
      },err=>{this.handleService.handleError(err)})
      .add(()=>{this.isDisabled=false;});
    }
  }
  ...
  cancelShit(update=false)
  {
    ...
    this.page = 1;
    this.validationForm8.patchValue({"haveOtherLocation":""});
    this.validationForm9.patchValue({"haveMoroccanProducts":""});
  }
